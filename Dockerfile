...
RUN apt update && apt install -y sbt gcc-riscv64-unknown-elf binutils-riscv64-unknown-elf scala gnupg curl git perl python3 make autoconf clang llvm lld cmake flex bison libgoogle-perftools-dev numactl perl-doc
WORKDIR /usr/src/
...
# Clone repositories
WORKDIR /root
ADD https://api.github.com/repos/riscv-non-isa/riscv-arch-test/git/refs/heads/main .riscv-arch-test-git-info.json
