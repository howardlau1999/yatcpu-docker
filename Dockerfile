...
RUN apt clean && apt update && apt install -y git perl python3 make autoconf g++ flex bison libgoogle-perftools-dev numactl perl-doc
WORKDIR /usr/src/
...
RUN apt install -y scala gnupg curl
